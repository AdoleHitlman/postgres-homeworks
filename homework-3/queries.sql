...
AND products.category_id IN (
    SELECT category_id FROM categories WHERE category_name = 'Dairy Products'
    UNION
...
FROM products
WHERE product_id IN (SELECT product_id FROM order_details WHERE quantity = 10) ORDER BY product_name;