...
SELECT country, COUNT(*) AS supplier_count FROM suppliers GROUP BY country ORDER BY COUNT(*) DESC;
...
SELECT ship_country, SUM(freight) AS total_weight FROM orders WHERE ship_region IS NOT NULL GROUP BY ship_country HAVING SUM(freight) > 2750 ORDER BY SUM(freight) DESC;
